...
public class Order {
    ...
    private Long productId; // 주문 대상 상품 ID (Product 엔티티 참조)

    @Column(name = "config_id", nullable = false)
    private Long configId; // OCSA 가중치 설정 ID (OcsaWeightConfig 엔티티 참조)
    ...
    private Boolean isFirstOrder; // 해당 상품의 최초 주문 여부 (alpha 신규성 반영에 사용)

    @Enumerated(EnumType.STRING)
    @Column(name = "order_status", nullable = false)
    private OrderStatus status; // 주문 처리 상태

    // 이하 OCSA 난이도 구성 지표 (Operational Complexity & Skill Assessment)
    ...
    private BigDecimal v3CapacityRequirements; // V3: 설비 역량 요구도

    @Column(name = "v4_space_time_urgency")
    private BigDecimal v4SpaceTimeUrgency; // V4: 공간·시간 긴급도

    @Column(name = "alpha_novelty")
    private BigDecimal alphaNovlety; // α: 신규성 보정 계수 (최초 주문일수록 높음)
    ...
    /** 순수 Java 테스트용 필드 (DB 컬럼 없음) */
    @Transient
    private String itemName; // 테스트 시에만 상품명을 직접 주입하여 사용

    /**
     * 일반 생성 시에는 초기 상태를 REGISTERED 로 설정한다.
     *
     * @param id          주문 PK
     * @param orderNumber 주문 번호
     * @param itemName    상품명(테스트용 Transient 필드)
    ...
    /**
     * 기술자 배정 확정 시 ANALYZED 상태에서만 호출 가능하며, 상태를 INPROGRESS 로 변경한다.
    ...
    /** 작업 완료 시 INPROGRESS 상태에서만 호출 가능하며, 상태를 COMPLETED 로 변경한다. */
    ...
    /** 배정 취소 시 INPROGRESS 상태에서 ANALYZED 로 롤백 */
    ...
    /**
     * Admin 에서 주문을 최초 등록할 때 사용하는 정적 팩토리 메서드.
     * 초기 상태는 항상 REGISTERED 로 고정된다.
     *
     * @param id           주문 PK (IdGenerator로 생성)
     * @param productId    주문 대상 상품 ID
     * @param configId     OCSA 가중치 설정 ID
     * @param orderNumber  사람이 읽을 수 있는 주문 번호
     * @param quantity     주문 수량
     * @param dueDate      납기 마감일
     * @param isFirstOrder 해당 상품의 최초 주문 여부
    ...
    /**
     * Admin 에서 주문 기본 정보를 수정한다.
     * REGISTERED 상태에서만 사용하며, SCM 워크플로우가 시작된 이후에는 수정 불가하다.
     *
