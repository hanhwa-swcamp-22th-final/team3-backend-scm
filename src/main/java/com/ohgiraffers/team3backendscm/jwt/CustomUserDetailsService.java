...
import com.ohgiraffers.team3backendscm.config.security.CustomUserDetails;
...
/**
 * JWT에서 추출한 사원코드(employeeCode)로 DB를 조회해 {@link CustomUserDetails}를 생성하는 서비스.
 *
 * <p>Admin 모듈에서 발행한 JWT의 {@code sub} 클레임은 {@code employee_code}(사원코드, String)이다.
 * SCM 도메인 서비스는 {@code employee_id}(Long, DB PK)를 사용하므로,
 * 이 서비스에서 두 값을 연결하는 역할을 한다.</p>
 */
@Service
...
public class CustomUserDetailsService implements UserDetailsService {
     ...
     * @return employeeId, employeeCode, role 등이 채워진 CustomUserDetails
