...
/**
 * HTTP 요청마다 Authorization 헤더에서 JWT를 추출해 인증 정보를 SecurityContext에 등록하는 필터.
 *
 ...
 *   <li>JWT sub(employeeCode)로 {@link UserDetailsService#loadUserByUsername} 호출 및 DB 조회</li>
        ...
        if (StringUtils.hasText(token)) {
            try {
                if (jwtTokenProvider.validateToken(token)) {
                    // 3. JWT sub(사원코드)로 DB에서 사용자 정보 로드 및 CustomUserDetails(employeeId 포함) 생성
